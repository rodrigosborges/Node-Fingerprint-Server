...
class FingerprintReader extends events.EventEmitter {
			...
			axios.get(`${this._configs.server}/api/funcionario/frequencia/biometricUsers`).then(response => {
				...
				fprint.identifyStart(this._deviceHandle, usersArray, ( state, message, index ) => {
					...
					if( message === 'identify-succeeded' ){
						...
						var cd = new Date();
    					var time = this.zeroPadding(cd.getDate(), 2) + '/' + this.zeroPadding(cd.getMonth()+1, 2) + '/' + this.zeroPadding(cd.getFullYear()) + ' ' + this.zeroPadding(cd.getHours(), 2) + ':' + this.zeroPadding(cd.getMinutes(), 2) + ':' + this.zeroPadding(cd.getSeconds(), 2);
						connection.emit('identify',{
							type: 'error',
							data: `${time}<br>Digital não localizada.`
						})
						setTimeout(() => {
							this._stopIdentify(() => {
								setTimeout(() => {
	...
	_getUsers( connection, page, search ) {
		this._debug('_getUsers');

		this._stopIdentify(() => {

			axios.get(`${this._configs.server}/api/funcionario/frequencia/list?page=${page}&search=${search}`)
